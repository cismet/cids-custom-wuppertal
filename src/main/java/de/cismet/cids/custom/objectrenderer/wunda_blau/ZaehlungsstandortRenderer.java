...
public class ZaehlungsstandortRenderer extends CoolPanel {
    private static final org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger("de.cismet.cids.objectrenderer.CoolPassantenfrequenzRenderer");
            ...
            Thread t = new Thread(new Runnable() {
                public void run() {
                    try {
                        ...
                        DefaultTableModel model = new DefaultTableModel(colNames, datum.size() + 1);
                        ...
                        for (int i = 0; i < datum.size(); i++) {
                            Date key = new Date(datum.get(i).getTime());
                        ...
                        for (Date s : tmAnzahl.keySet()) {
                            ...
                            int wert = tmAnzahl.get(s) * 12;
                            avg += wert;
                            model.setValueAt(DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT, Locale.GERMANY).format(s), i + 1, 0);
                            model.setValueAt(wert, i + 1, 1);
                            if (fillEreignis && tmEreignis.get(s) != null) {
                                model.setValueAt(tmEreignis.get(s), i + 1, tabFrequenzen.getColumn("Ereignis").getModelIndex());
                            }
                            if (fillWetter && tmWetter.get(s) != null) {
                                model.setValueAt(tmWetter.get(s), i + 1, tabFrequenzen.getColumn("Wetter").getModelIndex());
                            }
...
//                            jahr = jahr.substring(jahr.length()-4);
                            dataset.addValue(wert, "Daten", DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT, Locale.GERMANY).format(s));
                            i++;
                        }
                        avg = avg / datum.size();
                        model.setValueAt("Durchschnitt \u00D8", 0, 0);
                        model.setValueAt(Math.round(avg), 0, 1);
                        JFreeChart chart = createChart(dataset, avg);
...
//        plot.addDomainMarker(marker, Layer.BACKGROUND);
    ...
    class WetterRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
            try {
                if (value != null) {
                    String wetter = value.toString().toLowerCase();
...
//        public CustomBarRenderer3D(double xOffset, double yOffset)  {
...
//        }
        ...
        public Paint getItemPaint(int row, int column) {
            try {
                ...
                Date noon = new Date(date.getTime());
                noon.setHours(12);
                noon.setMinutes(0);
                noon.setSeconds(0);
                if (date.before(noon)) {
                ...
                if (date.toString().startsWith(STRING_MO)) {
                    if (paintLight) return LIGHT_MO;
                    else return COLOR_MO;
                } else if (date.toString().startsWith(STRING_DI)) {
                    if (paintLight) return LIGHT_DI;
                    else return COLOR_DI;
                } else if (date.toString().startsWith(STRING_MI)) {
                    if (paintLight) return LIGHT_MI;
                    else return COLOR_MI;
                } else if (date.toString().startsWith(STRING_DO)) {
                    if (paintLight) return LIGHT_DO;
                    else return COLOR_DO;
                } else if (date.toString().startsWith(STRING_FR)) {
                    if (paintLight) return LIGHT_FR;
                    else return COLOR_FR;
                } else if (date.toString().startsWith(STRING_SA)) {
                    if (paintLight) return LIGHT_SA;
                    else return COLOR_SA;
                } else if (date.toString().startsWith(STRING_SO)) {
                    if (paintLight) return LIGHT_SO;
                    else return COLOR_SO;
                } else {
                    return Color.BLACK;
                }
            } catch (Exception exception) {
