...
public class KundengruppeCExtProvider implements CExtProvider<CidsBeanAction> {

    //~ Static fields/initializers ---------------------------------------------

    private static final Logger LOG = Logger.getLogger(KundengruppeCExtProvider.class);
     ...
     */
    public KundengruppeCExtProvider() {
    ...
    public Collection<? extends CidsBeanAction> provideExtensions(final CExtContext context) {
        ...
        if (context != null) {
            ...
            if (((mc != null) && (ctxBean != null))
                        && ("billing_kundengruppe".equalsIgnoreCase(mc.getTableName()))) {
                final CidsBeanAction action1 = new ComposeEMailToKundenDirectContactAction();
                final CidsBeanAction action2 = new ComposeEMailToAllLoginsOfKundenAction();
