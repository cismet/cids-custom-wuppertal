...
public class EaStandortEditor extends DefaultCustomObjectEditor implements CidsBeanRenderer,
    ...
    private void initComponents() {
        ...
        btnRemStecker.setIcon(new ImageIcon(getClass().getResource("/de/cismet/cids/custom/objecteditors/wunda_blau/edit_remove_mini.png"))); // NOI18N
        ...
        binding = Bindings.createAutoBinding(AutoBinding.UpdateStrategy.READ_WRITE, this, ELProperty.create("${cidsBean.schnellladung}"), chSchnell, BeanProperty.create("selected"));
        ...
        binding = Bindings.createAutoBinding(AutoBinding.UpdateStrategy.READ_WRITE, this, ELProperty.create("${cidsBean.standortinformation}"), taStandort, BeanProperty.create("text"));
        bindingGroup.addBinding(binding);
        ...
        binding = Bindings.createAutoBinding(AutoBinding.UpdateStrategy.READ_WRITE, this, ELProperty.create("${cidsBean.anzahl_ladeplaetze}"), spPunkte, BeanProperty.create("value"));
        ...
        binding = Bindings.createAutoBinding(AutoBinding.UpdateStrategy.READ_WRITE, this, ELProperty.create("${cidsBean.anzahl_saeulen}"), spSaeule, BeanProperty.create("value"));
        binding.setSourceNullValue(0);
        binding.setSourceUnreadableValue(0);
        bindingGroup.addBinding(binding);
        ...
        binding = Bindings.createAutoBinding(AutoBinding.UpdateStrategy.READ_WRITE, this, ELProperty.create("${cidsBean.id_intern}"), txtIntern, BeanProperty.create("text"));
        bindingGroup.addBinding(binding);
    ...
    private void btnRemSteckerActionPerformed(ActionEvent evt) {//GEN-FIRST:event_btnRemSteckerActionPerformed
                ...
                for (int i = 0; i < listSteckerBeans.size(); i++) {
                    ...
                    if (listSteckerBeans.size() > (i + 1)) {
                        for (int j = i + 1; j < listSteckerBeans.size(); j++) {
                            if (
                                listSteckerBeans.get(i).getProperty(FIELD__STECKER_LEISTUNG).equals(listSteckerBeans.get(j).getProperty(FIELD__STECKER_LEISTUNG))
                                        && listSteckerBeans.get(i).getProperty(FIELD__STECKER_TYP).equals(listSteckerBeans.get(j).getProperty(FIELD__STECKER_TYP))) {
                                return 1;
                            }
                        }
                    }
