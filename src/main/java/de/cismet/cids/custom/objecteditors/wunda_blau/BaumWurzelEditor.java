...
public class BaumWurzelEditor extends DefaultCustomObjectEditor implements CidsBeanRenderer,
    ...
    private final RedundantObjectSearch wurzelSearch = new RedundantObjectSearch(
    ...
    /**
     * Creates a new BaumWurzelEditor object.
     *
    ...
    public boolean prepareForSave() {
        ...
        try {
            if (txtName.getText().trim().isEmpty()) {
                ...
                errorMessage.append(NbBundle.getMessage(BaumWurzelEditor.class, BUNDLE_NONAME));
            } else {
                if (redundantName) {
        ...
        if (errorMessage.length() > 0) {
            JOptionPane.showMessageDialog(StaticSwingTools.getParentFrame(this),
                NbBundle.getMessage(BaumWurzelEditor.class,
                    BUNDLE_PANE_PREFIX)
                        + errorMessage.toString()
                        + NbBundle.getMessage(BaumWurzelEditor.class,
                            BUNDLE_PANE_SUFFIX),
                NbBundle.getMessage(BaumWurzelEditor.class, BUNDLE_PANE_TITLE),
    ...
    private void valueFromOtherTable(final Collection<String> where) {
        final SwingWorker<Collection<MetaObjectNode>, Void> worker = new SwingWorker<Collection<MetaObjectNode>, Void>() {
            ...
            protected Collection<MetaObjectNode> doInBackground() throws Exception {
                wurzelSearch.setWhere(where);
                wurzelSearch.setTable(REDUNDANT_TABLE);
