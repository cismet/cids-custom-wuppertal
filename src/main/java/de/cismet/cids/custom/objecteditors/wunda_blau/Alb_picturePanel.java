...
//    private static final String[] MD5_PROPERTY_NAMES = new String[]{"lageplan_md5", "textblatt_md5"};
    ...
    public Alb_picturePanel(boolean selfPersisting) {
        ...
        if ((geometry != null) && (documentNo > NO_SELECTION) && (pageNo > NO_SELECTION)) {
            ...
            if ((oldVal == null) || !oldVal.equals(geometry)) {
                ...
                if (pageGeoCollection != null) {
                    ...
                    if (!pageFound) {
                        CidsBean newBean = CidsBeanSupport.createNewCidsBeanFromTableName(
...
//        currentActualDocumentMD5 = "";
    ...
    final class FileSearchWorker extends SwingWorker<List[], Void> {
        ...
        protected List[] doInBackground() throws Exception {
            ...
            if (blattObj != null) {
                result[TEXTBLATT_DOCUMENT] = BaulastenPictureFinder.findTextblattPicture(blattObj.toString().replaceAll("\\\\", "/"));
//                log.info("Blatt picture " + result[TEXTBLATT_DOCUMENT]);\\\
            }
            if (planObj != null) {
                result[LAGEPLAN_DOCUMENT] = BaulastenPictureFinder.findPlanPicture(planObj.toString().replaceAll("\\\\", "/"));
